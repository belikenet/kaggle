...
```{r}
factorVars <- c("OutcomeType", "AnimalType", "SexuponOutcome", "Breed", "Color", "Age.Unit", "Weekday", "Month", "Hour", "Breed.Main", "Color.Main", "Sex", "Sex.IsUnknown", "Sex.IsIntact", "Breed.Hair")
...
```{r}
trc.recv <- trainControl(method = "repeatedcv", repeats = 10, classProbs = TRUE)
```
...
model.summary <- function (train.model, data = test.batch, procCM = T) {
	...
	pred <- predict(train.model, data, type="vote")
	if (procCM) 
	{
		message("Confusion Matrix")
		print (confusionMatrix(pred, data$SexuponOutcome))
		#print (roc(as.numeric(pred), as.numeric(data$SexuponOutcome), auc=T))
	}
	return (pred)
}
```
...
```{r rf, cache=T}
train.rf <- train(OutcomeType ~ Name.NoName + AnimalType + Breed + Breed.IsMix + Breed.Hair + Breed.Main + Month + Year + Weekday + Age.Days + Sex + Sex.IsUnknown + Sex.IsIntact + Color.IsFlat + Color.H + Color.S + Color.V, data = train.batch, method = "rf", metric = "ROC", trControl = trc.recv)
```
