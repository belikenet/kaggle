...
This is my first attempt on data science. I love the Titanic challenge because there are several posts in the Internet which describe 
how to squeeze and process data coming from the Kaggle's Titanic datasets. Basically, I based my work on 
[Exploring the Titanic Dataset](https://www.kaggle.com/mrisdal/titanic/exploring-survival-on-the-titanic/notebook) and
[Titanic: Getting Started with R](http://trevorstephens.com/post/72916401642/titanic-getting-started-with-r), 
both of them are terrific examples about how to address several problems in these datasets. Besides, I used other non-scientific essays 
about the Titanic's catastrophe in order to find new answers. Please feel free to make any comment / suggestions, and forgive my _English_.
...
From now on, I will use variable names starting by `rows.*` for representing logical/int vectors, and they will be used as row dataframe filters.
...
| Fare | Ticket's price (british pounds) |
...
Less frequent titles will be grouped in a new Title category named `Noble`.
...
Another honorific titles in the passenger list came from another languages:

Dona
 ~ spanish title which is equivalent to Mrs (correct form is "Doña") <br/>
...
<span class="glyphicon glyphicon-eye-open"></span> <a href="#title_ss" class="button" data-toggle="collapse">safe check</a>

##### {#title_ss .collapse}

I made an special title category combining "Capt", "Col", "Dr", "Major", "Rev", "Sir" as these titles are bound to some _hierarchies_/jobs which may influence passenger's fate.
...
<span class="glyphicon glyphicon-info-sign"></span> <b>interesting facts</b>
...
Create a new predictor based on the sum of `Parch` and `SibSp`. May be the survival ratio shows some correlation with the size of the family (does the family sink or swim together?)
...
<span class="glyphicon glyphicon-eye-open"></span> <a href="#famsize_ss" class="button" data-toggle="collapse">safe check</a>
...
Small families seems to be favored against larger families and singles, but this need to be checked against other features (like Pclass, Title or Deck).
</div>

###Embarked

Checking out completeness of embarkation port, I discovered two passengers missing data in this column:

```{r }
(.lastVal <- combi[is.na(combi$Embarked),c("Name", "Pclass", "Ticket", "Fare")])
```

Let's try to figure out embarkation port checking the price paid by another passengers depending on the embarkation port:
...
After testing several imputation methods, MICE seems to recover missing values smoothly.
...
<span class="glyphicon glyphicon-eye-open"></span> <a href="#age_mice_ss" class="button" data-toggle="collapse">safe check</a>

##### {#age_mice_ss .collapse}

Let's review new age distribution density ... did it change old age distribution?
...
<span class="glyphicon glyphicon-eye-open"></span> <a href="#ageIs_o" class="button" data-toggle="collapse">safe check</a>

##### {#ageIs_o .collapse}

I know, I know .. we don't need to be so precise, but it makes me feel better.
Then, review survival ratios for `Age.IsChild` and `Age.IsMother` predictors by class
...
Against my personal believe, most of these _VIP_ passengers did not survive (may be because they view themselves as crew members, who most of them died as well)
...
Ticket fare mainly depended on class ticket (`Pclass`) and number of passengers travelling on the same ticket (`Family.Size`). 
Other factors affecting ticket fare: 
    
* children paid less for the ticket  
* there were family tickets to make the travel affordable to all kind of families
* the ticket also cover the trip from the origin city to the port
...
<div class="caption" style="margin-top: -8px">Third Class (Steerage) Passenger's Contract Ticket</div>
...
<span class="glyphicon glyphicon-eye-open"></span> <a href="#ticket_ss" class="button" data-toggle="collapse">safe check</a>
...
<span class="glyphicon glyphicon-eye-open"></span> <a href="#ticketName_ss" class="button" data-toggle="collapse">safe check</a>
...
ggplot() + 
    geom_point(aes(x = names(ticketNames.chisq), y = ticketNames.chisq)) + 
    xlab("is.factor") + 
    ylab ("chi.square test")
```

As expected, `Ticket.NameSimple` seems to be correlated with `Embarked`, `Pclass` and `Ticket.Name` features.
...
so let's check if there are cases which defy former hypothesis:
...
In conclusion, passengers on the same cabin known each other and share same ticket; although there exist a `r round(nrow(.lastVal)*100/nrow(combi))` percentage of weird cases, where cabins are shared amongst total strangers.
...
Last hypothesis to prove would be to test if all tickets are class orthogonal:

```{r}
combi %>% group_by(Ticket) %>% summarise(count = n_distinct(Pclass)) %>% filter (count > 1)
combi %>% group_by(Ticket.Name) %>% summarise(count = n_distinct(Pclass)) %>% filter (count > 1)
combi %>% group_by(Ticket.NameSimple) %>% summarise(count = n_distinct(Pclass)) %>% filter (count > 1)
```

Not only `Ticket`, but `Ticket.Name` and `Ticket.NameSimple` can be considered class orthogonal as well.
In the end, it looks like the hypothesis is correct, and passenger's ticket may be used as a location feature, but it won't be as good location feature as `Cabin`.
...
In my opinion, ticket field contains many typos (probably, bad transcriptions from original papers). 
The ticket may be used as well to combine families / groups travelling with the same ticket, and can be correlated as well with embarkation port.
All these facts could be correlated with the passenger's cabin, which in most cases is unknown.
</div>
...
so I create another feature named `Ticket.Count`.
...
<span class="glyphicon glyphicon-eye-open"></span> <a href="#ticketCount_ss" class="button" data-toggle="collapse">safe check</a>
...
ticket.count.sc <- combi %>% 
    filter(Ticket.Count != Family.Size & Ticket.Count == 3) %>% 
    select(Name, Pclass, Embarked, Family.Size, Ticket.Count, Ticket, Ticket.Name) %>% 
    arrange(Ticket)
kable(ticket.count.sc, col.names = c("Name","Class","Embarked","FamSize","TickCount","Ticket","TickName"))
```

```{r include=F}
remove(ticket.count.sc)
```

In order to show the differences between ticket count and family size, I created a query returning passengers `Ticket.Count != Family.Size & Ticket.Count == 3`.
Then , I research on first passenger on the list: [Cherry, Miss. Gladys](http://www.encyclopedia-titanica.org/titanic-survivor/gladys-cherry.html). 
...
<span class="glyphicon glyphicon-eye-open"></span> <a href="#ticketCountFare_ss" class="button" data-toggle="collapse">safe check</a>
...
ticket.count.sc <- combi %>% 
    filter (Fare > 100) %>% 
    select (Name,Fare,Fare.PerPerson,Family.Size,Fare.PerTicket,Ticket.Count,Pclass,Ticket) %>% 
    arrange (Ticket, Fare)

kable(ticket.count.sc)
...
We observe a similar distribution between `Fare.PerPerson` and `Fare.PerTicket`
...
`Ticket.Count` / `Fare.PerTicket` and `Family.Size` / `Fare.PerPerson` are ones of the most important variables in several machine learning algorithms I tested.
`Fare.PerTicket` returns reasonable per person price in cases where `Fare.PerPerson` looked suspicious.
</div>
...
I will create another codification (`Family.NameSimple`), combining in a single group passengers 
with family size below three and frequency below 3 (this _artificial_ cluster is named "Small").
...
It's very unlikely we can imputate cabins from current data. My approach will be obtaining a _likely_ cabin,
and then use this data to infer `Deck` (in other words, we won't use directly this new cabin data).

The first imputation we will use `Ticket` information, 
as probably tickets belonging to the same class and ticket, would occupy _nearby_ cabins. We already have seen, that 
there are some cases of same-ticket passengers spread along several cabins, 
but even in those cases, they stay on the same deck and most of the times, in contiguous cabins.
...
<span class="glyphicon glyphicon-eye-open"></span> <a href="#cabinTicket_ss" class="button" data-toggle="collapse">safe check</a>
...
This last refinement is not needed if using the data for deck inference, but if we want to infer the side of the cabin, this will help a bit.
...
In conclusion, using this _prediction rule_, we figure out `r .lastVal` cabin values. 


The second trick will be based on `Fare`, `Pclass` and `Family.Size` data. 

```{r warning=F}
predictCabinByFare <- function(data, rows.filter) {
    ticket.fare <- data %>% 
		filter (!is.na(Cabin) & Fare > 0) %>%
        group_by(Fare.PerPerson, Embarked, Pclass, Family.Size, Cabin) %>%
        summarise(max=max(n())) %>%
        arrange(desc(max)) %>%
        summarize_each(funs(first)) %>%
		rename (Cabin.Ren = Cabin) %>%
		select (-max)

	rj <- right_join(ticket.fare, data)
	return (rj$Cabin.Ren[rows.filter])
}
```

<span class="glyphicon glyphicon-eye-open"></span> <a href="#cabinFareFunc_ss" class="button" data-toggle="collapse">safe check</a>

##### {#cabinFareFunc_ss .collapse}

Let's check my point about using `predictCabinByFare`:

```{r warning=F}
cabinFareSafeCheck <- combi %>% 
    filter (!is.na(Cabin) & Fare > 0) %>%
    group_by(Fare.PerPerson, Embarked, Pclass, Family.Size, Cabin) %>%
    summarise(max = max(n()))
print (cabinFareSafeCheck, n = 30) 
```

The price of the ticket depends on several features, but most important ones are: class, embarkation port and family size. 

```{r warning=F}
cabinFareSafeCheck <- cabinFareSafeCheck %>% 
    arrange(desc(max)) %>%
    summarize_each(funs(first))
print (cabinFareSafeCheck , n = 30) 	
```
...
```{r include=F}
rm(cabinFareSafeCheck)
```

As there can be several clusters in the same group, I select the first most common one.

<a href="#cabinFareFunc_ss" data-toggle="collapse" class="text-right btn-block glyphicon glyphicon-remove"></a>
...
# use following prediction rule at your own risk
cabin.extended[rows.fix.cabin.by.fare] <- predictCabinByFare(combi, rows.fix.cabin.by.fare)
```

<span class="glyphicon glyphicon-eye-open"></span> <a href="#cabinFare_ss" class="button" data-toggle="collapse">safe check</a>
...
Using last _prediction rule_, we infer another `r .lastVal` values .... (every little helps). 
This prediction rule is very aggresive, and I didn't have the time to test their correctness.


Finally, I will move this new data into a new column named `Cabin.Extended` and this data will be used to generate `Side` and `Deck` information.
...
<span class="glyphicon glyphicon-info-sign"></span> <b>interesting facts</b>
```{r warning=F}
combi %>% group_by(Pclass) %>% summarise(count = n_distinct(Cabin))
```
Cabin data seems to be available for passengers in first class, but for second and third classes is more scarce.
</div>
...
```{r include=F}
rm(deck)
.lastVal = round(prop.table(table(rows.fix.cabin.unk)) * 100, 2)
```

As mentioned before, `Deck` can be extracted from `Cabin` data. Problem is that cabin information is scarce (`r .lastVal[["FALSE"]]`% of known values), 
so I've got my own doubts about its usefulness.

```{r warning=F}
plot(table(combi$Deck[rows.train], combi$Pclass[rows.train]), dnn=c("Deck","Class"), main="Class / Deck")
boxplot(Fare ~ Deck, combi[rows.train & combi$Fare < 100 & combi$Fare > 0,])
```
Looking at previous plots, there is a single passenger which deck seems not valid:
...
Using last boxplot, we know that most of the unknown cabins paid less than 20 pounds, 
...
After applying last rule, boxplot shows the flow of passengers from unknown deck to F deck.

<div class="alert alert-info">
<span class="glyphicon glyphicon-info-sign"></span> <b>interesting facts</b>
```{r}
# deck frequency table (%)
(.lastVal <- round(prop.table(table(combi$Deck)) * 100, 2))
```
Applying former rules, `r 100 - as.numeric(.lastVal[["UNK"]])`% of the passenger's deck cabin location can be inferred.
</div>

###Side
Cabin is encoded like a letter (corresponding to the deck) followed by the cabin number, 
which (by convention) is odd for cabins on the starboard side and even for the port side.
There is an exception to this convention: in deck E, passengers cabins for first and second class
...
The problem is that cabin data is not complete neither normalized. 
Thus, best case is when the cabin value is like "A26", which univocally identify deck and cabin. 
...
<span class="glyphicon glyphicon-eye-open"></span> <a href="#side_ss" class="button" data-toggle="collapse">safe check</a>
...
If there is no cabin data, or the data doesn't contain a proper cabin number (for example, passengers on cabin "D"), we set the side probability to NaN. 

Let's review some special cases ... how many "cabins" contains spaces?

```{r}
unique(grep(" ", combi$Cabin.Extended, value=T))
```

Cabins in former case are contiguous (`"B96 B98"` or `"D10 D12"`). There is a single case
where the side probability is _special_ (cabin `"B57 B59 B63 B66"`), 
as this "cabin" mixes odd and even cabins. In this case, the ratio between odd and even is the side probability (3/4).

```{r}
combi %>% 
    filter (Cabin.Extended %in% c("B57 B59 B63 B66","F G73","B96 B98")) %>% 
    select (Pclass, Ticket, Cabin.Extended, Side, Side.Prob) %>% 
    arrange (Ticket, Cabin.Extended)
```
...
```{r include=F}
.lastVal = sum(is.na(combi$Cabin.Extended) | is.nan(combi$Side.Prob))
```

Unknown side category values (`r .lastVal`) came from passengers with `Cabin` equal to NA or `Side.Prob` equal to NaN. 
As explained before, side probabilities to NaN is for cabins with no cabin number:
...
<span class="glyphicon glyphicon-info-sign"></span> <b>interesting facts</b>
```{r include=F}
sideByCabin <- predictSide(combi %>% rowwise() %>% mutate (Side.Prob = inferSideProb(Cabin)))
tf.sideByCabin <- prop.table(table(sideByCabin$Survived[rows.train], sideByCabin$Side[rows.train])) * 100
rm(sideByCabin)
```
```{r}
# side frequency by cabin
print(tf.sideByCabin)
# side frequency by cabin extended
prop.table(table(combi$Survived[rows.train], combi$Side[rows.train])) * 100
```

The difference between Sides (starboard / portside) is minimal. But several 
[essays](http://www.williammurdoch.net/man-08_starboard_evacuation.html) confirm 
that the evacuation in the starboard side was more efficient than in the portside.
</div>
...
I tested several predictions methods, and I've got the best results using C5.0
...
```{r pred, cache=T, warning=F}
trc.recv <- trainControl(method = "repeatedcv", repeats = 10, summaryFunction = twoClassSummary, classProbs = TRUE)
# training using training holdout
train.c5.best <- train(Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Family.NameSimple, train.batch, method = "C5.0", metric = "ROC", trControl = trc.recv, tuneLength=5)
```

```{r pred_vi, cache=T}
vi <- varImp(train.c5.best)
vi.df <- data.frame(
    variables = row.names(vi$importance), 
    importance = vi$importance$Overall, 
    rank = paste0("#", dense_rank(desc(vi$importance$Overall))))
ggplot(vi.df[vi.df$importance > 5,], aes(x = reorder(variables, importance), y = importance, fill = importance)) + 
    geom_bar(stat = "identity") + 
    geom_text(aes(x = variables, y = 0.5, label= rank), vjust = 0.55, hjust = 0, size = 4) + 
    labs(x="Features") + 
    coord_flip() +
    theme_few()
```
...
```{r pred_cv, cache=T}
pred <- predict(train.c5.best, test.batch)
(.lastVal <- confusionMatrix(pred, test.batch$Fate, positive="survived"))
#plot(roc(as.numeric(pred), as.numeric(test.batch$Fate), auc=T))
```

The accuracy using test batch was about `r round(.lastVal$overall[["Accuracy"]] * 100, 2)`. It is good, but it looks like I'm overfitting, 
because when uploading results to kaggle using the whole training set, I'm getting much worse predictions:

```{r pred_full,cache=T,warning=F}
# full model using whole training data
train.c5.best <- train(Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Family.NameSimple, train.pre, method = "C5.0", metric = "ROC", trControl = trc.recv, tuneLength=5)
pred <- predict(train.c5.best, test.pre)
result <- data.frame(Survived = as.numeric(pred) - 1, PassengerId = test.pre$PassengerId)
...
Do I miss something? Anyway, it's being a long journey, with a lot of learnings. 
As I said at the beginning, any comment or suggestion will be very much appreciated. Thanks for the reading.
...
```{r rf_3b, cache=T}
train.rf <- train(Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Ticket.Name, train.pre, method = "rf", metric = "ROC", trControl = trc.recv, tuneLength=5)
```
...
#results.rf <- model.summary(train.rf, test.pre, F)
#result <- data.frame(Survived = as.numeric(results.rf) - 1, PassengerId = test.pre$PassengerId)
#write.csv(result, "../titanic/titanic.csv", quote=F, row.names=F)
```
#####

```{r rf_3, cache=T}
train.rf <- train(Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Ticket.Name, train.batch, method = "rf", metric = "ROC", trControl = trc.recv, tuneLength=5)
```
...
```{r rf_6, cache=T}
train.rf.best <- train(Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age, train.batch, method = "rf", metric = "ROC", trControl = trc.recv, tuneLength=5)
```
...
train.c5 <- train(Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Family.NameSimple + Embarked, train.batch, method = "C5.0", metric = "ROC", trControl = trc.recv, tuneLength=5)
```
...
|-----------------------------------------------------------------------------------------------------------|----:|---------:|-------:|------------|
| **`Fate ~ Sex + Pclass + Fare + Sex:Pclass + Family.Size + Title + Age`** | 25 | 0.8596 | 0.8547 |    |

**KNN**

| formula | bad | accuracy | AUC | preProcess |
|-----------------------------------------------------------------------------------------------------------|----:|---------:|-------:|------------|
| **`Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Family.NameSimple`** | 29 | 0.8371 | 0.8319 |  |
...
| `Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Deck + Age.IsMother + Age.IsChild` | 27 | 0.8483 | 0.834 |  |
| `Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Ticket.Name` | 25 | 0.8596 | 0.8482 |  |
| `Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Ticket.Name + Age.IsChild` | 27 | 0.8483 | 0.8375 |  |
| `Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count` | 23 | 0.8708 | 0.8605 |  |
| **`Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age`** | 23 | 0.8708 | 0.8605 |  |

**CFOREST**

| formula | bad | accuracy | AUC | preProcess |
|-----------------------------------------------------------------------------------------------------------|----:|---------:|-------:|------------|
| `Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Ticket.Name` | 25 | 0.8596 | 0.8522 |  |

**C5.0**

| formula | bad | accuracy | AUC | preProcess |
|-----------------------------------------------------------------------------------------------------------|----:|---------:|-------:|------------|
| `Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Ticket.Name` | 29 | 0.8371 | 0.8269 |  |
| **`Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Family.NameSimple`** | 25 | 0.8596 | 0.8469 |  |
| `Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Family.NameSimple + Embarked` | 28 | 0.8427 | 0.8322 |  |
| `Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Family.NameSimple + Embarked` | 25 | 0.8596 | 0.8469 |  |
| `Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Family.NameSimple + Embarked` | 25 | 0.8596 | 0.8469 |  |
...
| formula | bad | accuracy | AUC | preProcess |
|-----------------------------------------------------------------------------------------------------------|----:|---------:|-------:|------------|
| `Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Family.NameSimple + Embarked` | 23 | 0.8708 | 0.8649 |  |
| **`Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Embarked + Deck`** | 20 | 0.8876 | 0.8765 | shrinkage = 0.06319474, n.trees = c(150,200), interaction.depth = c(5, 6, 7), n.minobsinnode = c(1, 5) |
| `Fate ~ Sex + Pclass + Fare.PerPerson + Family.Size + Title + Age + Ticket.Count + Embarked + Deck` | 26 | 0.8539 | 0.8429 | shrinkage = 0.06319474, n.trees = c(150), interaction.depth = c(5,7,9), n.minobsinnode = c(1) |
...
```{r dependson="stack2", eval=F}
pred.ens <- predict(car.ens, test.batch,type="raw")
confusionMatrix(pred.ens, test.batch$Fate, positive="survived")
```
#### stack3

```{r stack3, cache=T, warning=F, eval=F}
...
```{r stack4, cache=T, warning=F, eval=F}
tuneList <- list (
...
```{r splitbypclass_o, dependson="splitbypclass", cache=T, eval=F}
result <- model.split.by.pclass (train.batch, test.batch)
```

```{r splitbypclassrf, eval=F}
...
```{r splitbypclassrfc5_o, dependson="splitbypclassrfc5", cache=T, eval=F}
result <- model.split.by.pclass_rf (train.batch, test.batch)
```

```{r splitbypclassgbmc5, eval=F}
...
```{r splitbypclassgbmc5_o, dependson="splitbypclassgbmc5", cache=T, eval=F}
